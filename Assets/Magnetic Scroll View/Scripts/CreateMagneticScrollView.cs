...
namespace MagneticScrollView
{
    public class CreateMagneticScrollView
    {
        ...
        public static void CreateScrollViewObject ()
        {
            ...
            if (selection == null)
            {
                selection = new GameObject ("Canvas", typeof (Canvas));
                selection.layer = 5;
                selectionCanvas = selection.GetComponent<Canvas> ();
            }
            else
            {
                selectionCanvas = selection.GetComponentInParent<Canvas> ();
                if (selectionCanvas == null)
                    selectionCanvas = selection.AddComponent<Canvas> ();
            }

            if (selectionCanvas != null)
            {
                GameObject selectionCanvasGO = selectionCanvas.gameObject;
                CanvasScaler canvasScaler = selectionCanvasGO.GetComponent<CanvasScaler> ();
                if (canvasScaler == null)
                    canvasScaler = selectionCanvasGO.AddComponent<CanvasScaler> ();

                GraphicRaycaster raycaster = selectionCanvasGO.GetComponent<GraphicRaycaster> ();
                if (raycaster == null)
                    raycaster = selectionCanvasGO.AddComponent<GraphicRaycaster> ();
                ...
                else
                {
                    ...
                    selectionCanvas.worldCamera = Object.FindObjectOfType<Camera> ();
                }
            }
            ...
            scrollView.transform.SetParent (selection.transform, false);
            scrollView.layer = 5;
